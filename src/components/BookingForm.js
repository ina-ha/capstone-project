...
const displayDetails = () => {setDetails(!details);
...
setConfirmed(true)}
 ...
 function handlePlus() {
  setGuests(prev => {if (prev < 10) {return prev + 1}
else {return prev}})
}

function handleMinus() {
  setGuests(prev => {if (prev > 1) {return prev - 1}
else {return prev}})
}
...
<option value='' disabled hidden >Select</option>
   ...
   (<div>
    ...
    <input id="firstName" style={validFirstName === 'invalid' ? {border: '1px solid red'} : {border: '1px solid black'}} value={firstName} onBlur={() => validateName(firstName, setValidFirstName)} onChange={e => {clearErrorName(e.target.value, setValidFirstName);
    setFirstName(e.target.value); }} type="text" minlength='2'></input>
    </label>
